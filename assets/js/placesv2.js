...
$(function () {
	setTimeout(function () {
		$('#showmapta').fadeIn(2500);	
	}, 5000);
	setTimeout(function () {
		$('.showmaptag').addClass('navbar',5000,"easeInSine");
	}, 5000);
	$('.create-maptag').on('click',function() {

		var request = $.ajax({
	        url: "http://mapta.gs/user/isactiveuser",
	        type: "POST",
	        dataType: "JSON"
	    ...
	    request.done(function (msg) {
	    	if(msg.code==1) {
	    		$(location).attr('href','http://mapta.gs/');
	    	} else {
	    		$(location).attr('href','http://mapta.gs/user/login');
	    	}
	    });
	});
